...
exports.getNewsData = async (req, res) => {
  ...
  try {
    ...
    const token = encodeURIComponent(process.env.NEWS_API_KEY);
    const query = encodeURIComponent('business');
    const page = encodeURIComponent(req.params.page);

    const url2 = `https://newsapi.org/v2/everything?q=${query}&sortBy=publishedAt&apiKey=${token}&language=en&pageSize=6&page=${page}`;
